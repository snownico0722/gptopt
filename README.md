...
- **横置遥控触控优化**：横置 iframe 内把整个 UI 略微放大（根字号 17px），输入框高度约 82px、底栏约 96px、发送/附件按钮约 58px，顶部/侧栏/消息操作按钮和菜单项同步加大，滚动条加宽到 18px；不做 transform/zoom 整页缩放，避免重新引入发虚。
- 内置一套 **轻量会话快捷导航**：
  - 普通 / 未横置模式：保留之前的紧凑横条导航，点 `≡` 展开问题列表；
  - 横置遥控模式：改为右侧**常驻文字导航**，直接显示“序号 + 问题文字”，每个问题整行都是大点击区；
  - 横置文字导航**不显示“会话导航 · N”标题**；
  - 横置文字导航**没有面板衬底、边框、阴影和模糊背景**，只保留文字与当前项强调线；
  - 当前阅读轮次会自动高亮；
  - 缓存已经看到过的问题标题；
